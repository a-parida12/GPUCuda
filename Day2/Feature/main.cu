...
//#define CAMERA
__device__ float EigVal1 (float a, float b, float c, float d)
{
    float T = a + d;
    float D = a*d-b*c;
    return T/2 - sqrtf(T*T/4-D);
}

__device__ float EigVal2 (float a, float b, float c, float d)
{
    float T = a + d;
    float D = a*d-b*c;
    return T/2 + sqrtf(T*T/4-D);
...
__global__ void EigenValTensor (float*out,float *m11,float *m12,float *m22, int w, int h,int nc, float alpha, float beta){
	...
	int currentlocation=ix + iy * w ;
	int currentlocation_image=ix + iy * w + iz*w*h;

	 if (ix<w && iy<h && iz < nc){ 
        float eig1 = EigVal1(m11[currentlocation], m12[currentlocation], m12[currentlocation], m22[currentlocation]);
        float eig2 = EigVal2(m11[currentlocation], m12[currentlocation], m12[currentlocation], m22[currentlocation]);
        if (alpha<=eig1 && iz == 0) {
            out[currentlocation_image] =  1;
        } 
		
		else if (eig2 >= alpha && eig1 <alpha && iz != 2){
            out[currentlocation_image] = 1;
        } 
		
		else 
            out[currentlocation_image] *= 0.5;
    }
...
int main(int argc, char **argv)
{
   ...
   showImage("Output Tensor m11 GPU", mOut_gray, 200, 100);
	...
	float alpha=0.001;
	float beta=0.0001;
	cudaMemcpy(g_imgOut, imgIn, h * w * nc* sizeof(float), cudaMemcpyHostToDevice );CUDA_CHECK;
	EigenValTensor <<<Grid,Block>>> (g_imgOut,g_ten_m11,g_ten_m12,g_ten_m22, w, h,nc, alpha, beta);CUDA_CHECK;
	cudaMemcpy(imgIn,g_imgOut, h*w * nc* sizeof(float), cudaMemcpyDeviceToHost );
   	CUDA_CHECK;
	...
	cudaFree(g_ten_m12);CUDA_CHECK;
	cudaFree(g_ten_m22);CUDA_CHECK;
    cudaFree(g_imgIn);
    CUDA_CHECK;
    cudaFree(g_gKernel);
    CUDA_CHECK;
