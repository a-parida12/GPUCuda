...
__global__ void cuda_gradient(float* d_imgIn,float* d_imgOut_gradx,float* d_imgOut_grady, int n, int h, int w)
{
 ...
 if(idx+1<n){
		...
		d_imgOut_gradx[idx]=d_imgIn[idx];
		d_imgOut_grady[idx]=d_imgIn[idx];}
...
__global__ void cuda_divergence(float* d_imgOut_gradx,float* d_imgOut_grady, float* d_imgOut_div,int n, int h, int w)
{
 ...
 if(idx+1<n){
		d_imgOut_div[idx]=d_imgOut_gradx[idx+1]-d_imgOut_gradx[idx]+d_imgOut_grady[idx+w]-d_imgOut_grady[idx];
}
 else 
		d_imgOut_div[idx]=d_imgOut_gradx[idx]+d_imgOut_grady[idx];
}
